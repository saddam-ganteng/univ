...
<div class="min-h-screen bg-gradient-to-b from-white via-stone-50 to-amber-50 px-6 py-16 pt-30">
    <div class="max-w-3xl mx-auto space-y-8">
        ...
        <?php else: ?>
            <img src="<?= $supabaseStorageUrl . htmlspecialchars($jurnal['gambar']) ?>" class="w-full h-64 object-cover rounded-xl shadow"
                alt="Cover Jurnal">

            <h1 class="text-3xl font-bold text-stone-800"><?= htmlspecialchars($jurnal['judul']) ?></h1>
            <p class="text-stone-500">
                Oleh <span
                    class="font-medium"><?= htmlspecialchars($jurnal['profile']['nama'] ?? 'Tidak Diketahui') ?></span> —
